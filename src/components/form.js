import React from 'react'
import {useState} from 'react'

 function Form({getData}){
    const [weight, setWeight] = useState("")
    const [height, setHeight] = useState("")
    const [alert, setAlert] = useState(false)
 
    const getHeight=(e)=>{
        setHeight(e.target.value)
    }
    const onSubmit=(e)=>{
        e.preventDefault();
        if(isNaN(weight)||isNaN(height)){
          setAlert(true)
            console.log('not a valid input');
        }else{
          getData(weight,height)
            console.log(height);
           console.log(weight);
           setHeight('');
           setWeight('');
        }
        
    }
    let alertMessage;
    /*if(alert){
      alertMessage= <div className='alert alert-danger mt-3 col-md-10'>Please enter valid data</div>
    }else{
      alertMessage=''
    }*/
    return(
      <div>
        <div className='d-flex justify-content-center mt-5 shadow  w-25 mx-auto p-3'>
          <div className='row mx-auto'>
            <div className='col-md-12'>
                <h4>BMI CALCULATOR</h4>
            </div>
            <form onSubmit={onSubmit}>
            <div className='col-md-6 '>
                <label className='form-label'>Weight</label>
                <input type='text' value={weight} onChange={(e)=>setWeight(e.target.value)} className='form-control' required/>
            </div>
            <div className='col-md-6'>
                <label className='form-label'>Height</label>
                <input type='text' className='form-control'  value={height} onChange={getHeight} required/>
            </div>
            <div className='col-md-12 mt-4'>
                <input type='submit' className='btn btn-primary' value="Get Bmi"/>
            </div>
            </form>
           {/* {alert?<div className='alert alert-danger mt-3 col-md-10'>Please enter valid data</div>:''} */
           alert && <div className='alert alert-danger mt-3 col-md-10'>Please enter valid data</div>
           }


          </div>
        </  div>
      </div>
    )
}

export default Form